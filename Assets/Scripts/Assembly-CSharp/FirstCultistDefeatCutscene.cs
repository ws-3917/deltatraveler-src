			...
			if (state == 3)
			{
				...
				else if (!txt)
				{
					...
					if (frames == 30)
					{
						...
						susie.ChangeDirection(Vector2.down);
						noelle.ChangeDirection(Vector2.up);
						StartText(new string[1] { "* Let's just go." }, new string[1] { "snd_txtsus" }, new int[18], new string[1] { "su_neutral" }, 1);
						state = 4;
					}
				}
			}
