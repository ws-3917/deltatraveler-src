...
public class JerrySlashBlue : BulletBase
{
	...
	protected override void Awake()
	{
		...
		sr.color = new Color32(0, 60, byte.MaxValue, byte.MaxValue);
	}

	private void Update()
	{
		frames++;
		sr.color = Color.Lerp(new Color32(0, 60, byte.MaxValue, byte.MaxValue), Color.white, (float)frames / 10f);
		...
		if (Mathf.Sign(base.transform.position.x) == Mathf.Sign(vector.x) && Mathf.Abs(base.transform.position.x) > 8f)
		{
